...
export const PlantSelector: React.FC<PlantSelectorProps> = ({ selectedPlants, onTogglePlant, onStartGame, onBack, levelConfig, unlockedPlants, isEndless, onSaveAndQuit }) => {
  ...
  // Get unique Zombies for display
  const levelZombies = Array.from(new Set(levelConfig.enabledZombies || []));

  return (
    <div className="absolute inset-0 z-50 bg-slate-900 flex flex-col items-center justify-center p-4 overflow-hidden">
      <div className="w-[1100px] bg-yellow-900/20 rounded-xl border-4 border-yellow-900/50 p-6 shadow-2xl backdrop-blur-sm flex flex-col gap-6">
        ...
        {/* Selected Bar */}
        <div className="h-24 bg-slate-800 rounded-lg border-2 border-slate-600 flex items-center px-2 gap-2 overflow-hidden shadow-inner justify-center">
           {selectedPlants.map(type => (
             <div 
               key={type} 
               onClick={() => onTogglePlant(type)}
               className="w-16 h-20 bg-green-900/80 border-2 border-green-500 rounded cursor-pointer hover:bg-red-900/50 flex flex-col items-center justify-center group"
             >
           ...
           {Array.from({ length: Math.max(0, maxSeeds - selectedPlants.length) }).map((_, i) => (
             <div key={`empty-${i}`} className="w-16 h-20 bg-black/20 border-2 border-dashed border-slate-700 rounded" />
           ))}
        </div>
        ...
        <div className="flex gap-6 h-[400px]">
                  ...
                  {allPlants.map(plant => {
                    ...
                    return (
                      <div
                        ...
                        className={`
                          relative w-20 h-24 rounded border-2 flex flex-col items-center justify-center transition-all duration-100
                          ${!isUnlocked 
            ...
            {/* Right: Level Info */}
            <div className="w-64 bg-slate-900/80 rounded-lg border-2 border-slate-600 p-4 flex flex-col">
                <div className="text-center border-b border-slate-700 pb-2 mb-4">
                    ...
                    <div className="text-slate-400 text-[10px] font-mono mt-1">{getSceneName(levelConfig.scene)}</div>
                </div>
