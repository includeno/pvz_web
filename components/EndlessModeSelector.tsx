...
export const EndlessModeSelector: React.FC<EndlessModeSelectorProps> = ({ onBack, onStartGame, language }) => {
      ...
      if (isCreating === slotId) {
          return (
              <div key={slotId} className="bg-slate-800 border-4 border-blue-500 rounded-xl p-6 flex flex-col gap-4">
                  <h3 className="text-xl text-blue-300 font-pixel">{t('NEW_GAME_CONFIG', language)}</h3>
                  <div className="text-xs text-slate-400">{t('SELECT_DLCS', language)}</div>
                  ...
                  <div className="flex gap-2 mt-auto">
                      ...
                      <button onClick={() => handleCreateSave(slotId)} className="flex-1 py-2 bg-green-600 text-white font-bold rounded shadow-lg">{t('START', language)}</button>
                  </div>
              </div>
          );
      }
          ...
          return (
              <div key={slotId} className="bg-slate-800 border-4 border-slate-600 rounded-xl p-6 flex flex-col gap-2 relative group">
                  <div className="flex justify-between items-start">
                      <div>
                          <h3 className="text-2xl text-yellow-400 font-pixel">{t('FLOOR', language)} {save.floor}</h3>
                          <div className="text-xs text-slate-400 font-mono">{t('SCORE', language)}: {save.score.toLocaleString()}</div>
  ...
  return (
    <div className="absolute inset-0 z-[2000] bg-slate-950 flex flex-col items-center justify-center p-8">
        <h2 className="text-4xl text-purple-400 font-pixel mb-8 drop-shadow-[0_4px_0_#000]">{t('ENDLESS_MODE', language)}</h2>
